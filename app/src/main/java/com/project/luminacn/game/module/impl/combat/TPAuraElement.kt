...
class TPAuraElement(iconResId: Int = AssetManager.getAsset("ic_dice_3_outline_black_24dp")) : Element(
    ...
    private var teleportBehind by boolValue("传送到背后", false)
    private var tpDelay by intValue("TP延迟", 500, 100..2000)
    private var targetDistance by floatValue("距离", 2.0f, 1f..5f)
    private var attackRange by floatValue("范围", 3.7f, 2f..20f)
    private var preserveY by boolValue("Y轴保持", true)
    private var packets by intValue("发包", 1, 1..10)  
