...
class GameDataManager {
    ...
    fun getPlayerListStats(): String {
        ...
        stats.append("玩家列表:\n")
        stats.append("  总玩家数: ${currentPlayerList.size}\n")

        if (currentPlayerList.isNotEmpty()) {
            val roleStats = getPlayersWithRole()
            stats.append("  主机: ${roleStats["hosts"]?.size ?: 0}\n")
            stats.append("  教师: ${roleStats["teachers"]?.size ?: 0}\n")
            stats.append("  分客户端: ${roleStats["subClients"]?.size ?: 0}\n")
            stats.append("  常规玩家: ${roleStats["regular"]?.size ?: 0}\n")

            stats.append("  玩家:\n")
            currentPlayerList.values.forEach { player ->
                val roles = mutableListOf<String>()
                if (player.isHost) roles.add("主机")
                if (player.isTeacher) roles.add("教师")
                if (player.isSubClient) roles.add("分客户端")
