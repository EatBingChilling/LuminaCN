...
class QuickAttackElement(iconResId: Int = AssetManager.getAsset("ic_flash_black_24dp")) : Element(
    ...
    private var maxRange by floatValue("最大范围", 5f, 2f..30f)
    private var attackCps by intValue("CPS", 15, 1..20)
    private var attackPackets by intValue("发包", 1, 1..5)
    private var criticalHits by boolValue("刀爆", false)
    private var multiTarget by boolValue("多目标", false)
    private var maxTargets by intValue("最多目标", 2, 1..5)

    private var strikeDistance by floatValue("挥剑范围", 1.5f, 0.5f..3f)
    private var returnDelay by intValue("返回延迟", 100, 50..500)
    private var humanizeMovement by boolValue("仿人移动", true)
