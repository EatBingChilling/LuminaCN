...
class EspElement(iconResId: Int = AssetManager.getAsset("ic_eye_black_24dp")) : Element(
    ...
    private fun drawEntityBox(entity: Entity, viewProjMatrix: Matrix4f, screenWidth: Int, screenHeight: Int, canvas: Canvas) {
        ...
        boxVertices.forEach { vertex ->
            val screenPos = worldToScreen(vertex, viewProjMatrix, screenWidth, screenHeight) ?: return@forEach
    ...
    private fun draw2DBox(canvas: Canvas, paint: Paint, minX: Double, minY: Double, maxX: Double, maxY: Double) {
        ...
        canvas.drawRect(minX.toFloat() + padding, minY.toFloat() + padding, maxX.toFloat() - padding, maxY.toFloat() - padding, paint)
    }
    ...
    private fun draw3DBox(canvas: Canvas, paint: Paint, screenPositions: List<Vector2f>) {
        ...
        edges.forEach { (start, end) ->
            ...
            if (isOnScreen(startPos, canvas) && isOnScreen(endPos, canvas)) {
                val padding = paint.strokeWidth / 2
                canvas.drawLine(startPos.x.coerceIn(padding, canvas.width - padding), startPos.y.coerceIn(padding, canvas.height - padding), endPos.x.coerceIn(padding, canvas.width - padding), endPos.y.coerceIn(padding, canvas.height - padding), paint)
            }
        }
    }
    ...
    private fun drawCornerBox(canvas: Canvas, paint: Paint, minX: Double, minY: Double, maxX: Double, maxY: Double) {
        ...
        corners.forEachIndexed { i, (x, y) ->
            canvas.drawLine(x.toFloat(), y.toFloat(), x.toFloat() + if (i % 2 == 0) cornerLength else -cornerLength, y.toFloat(), paint)
            canvas.drawLine(x.toFloat(), y.toFloat(), x.toFloat(), y.toFloat() + if (i < 2) cornerLength else -cornerLength, paint)
        }
    }
    ...
    private fun drawEntityInfo(canvas: Canvas, entity: Entity, minX: Double, minY: Double, maxX: Double) {
        ...
        val padding = 8f
        bgRect.set(textX - textBounds.width() / 2 - padding, textY - textBounds.height() - padding, textX + textBounds.width() / 2 + padding, textY + padding)
